...
<template>
    <div>
        <h1>Componente para Estados de Usuario</h1>

        <h3>Crear un Estado de Usuario</h3>
        <input v-model="nuevo.id" placeholder="id" />
        <input v-model="nuevo.nombre" placeholder="nombre" />
        <input v-model="nuevo.permite_acceso" placeholder="permite_acceso (true/false)" />
        ...
        <table border="1">
            ...
            <tr v-for="e in estados" :key="e.id">
                ...
                <td><input v-model="e.permite_acceso" /></td>
...
export default {
    ...
    const vuelta = () => router.back();
        ...
        const cargar = async () => {
            const res = await fetch("http://44.207.19.239:3000/estados_usuario?zusuario=ethan");
        ...
        const crear = async () => {
            await fetch("http://44.207.19.239:3000/estados_usuario?zusuario=ethan", {
                method: "POST",
        ...
        const actualizar = async (e) => {
            await fetch(`http://44.207.19.239:3000/estados_usuario/${e.id}?zusuario=ethan`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
        ...
        return { estados, nuevo, crear, actualizar, borrar,vuelta };
    }
};
</script>
<style scoped>
